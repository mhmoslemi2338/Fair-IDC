...
class Synthesizer():
    ...
    def loader(self, args, augment=True):
        ...
        # --- 1. Add CIFAR10_S_90 ---
        elif args.dataset == 'CIFAR10_S_90':
            # Use the existing CIFAR transform. 
            # Note: Ensure MEANS['cifar'] is appropriate, or create a custom one if needed.
            train_transform, _ = transform_cifar(augment=augment, from_tensor=True)

        # --- 2. Add Colored Datasets ---
        elif args.dataset.startswith('Colored_MNIST') or args.dataset.startswith('Colored_FashionMNIST'):
    ...
    def test(self, args, val_loader, logger, bench=False):
...
def load_resized_data(args):
    ...
    if args.dataset == "CIFAR10_S_90":
        # Assuming cifar10.CIFAR_10s returns (train_ds, test_ds, mean, std)
        train_dataset, val_dataset, mean, std = cifar10.CIFAR_10s(args.data_dir, skew_ratio=0.9)
        normalize = transforms.Normalize(mean=mean, std=std)
        # Assuming the custom function handles transforms, but we define this for consistency
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
        
        train_dataset.nclass = 10
        train_dataset.channels = 3
        train_dataset.im_size = (32, 32)
        train_dataset.classes = ["ss"]

    elif args.dataset == 'Colored_MNIST_foreground':
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        normalize = transforms.Normalize(mean=mean, std=std)
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])

        # We discard the returned loader (_) to follow standard PyTorch patterns
        # where the loader is defined after the dataset block using args.batch_size
        train_dataset, _ = get_biased_mnist_dataloader(args.data_dir, batch_size=args.batch_size,
                                                data_label_correlation=0.9,
                                                is_foreground=True,
                                                n_confusing_labels=9,
                                                train=True)
        val_dataset, _ = get_biased_mnist_dataloader(args.data_dir, batch_size=args.batch_size,
        ...
        train_dataset.channels = 3
        train_dataset.im_size = (32, 32)
        train_dataset.classes = [str(c) for c in range(10)]

    elif args.dataset == 'Colored_FashionMNIST_foreground':
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        normalize = transforms.Normalize(mean=mean, std=std)
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])

        train_dataset, _ = get_biased_fashionmnist_dataloader(args.data_dir, batch_size=args.batch_size,
                                                data_label_correlation=0.9,
                                                is_foreground=True,
                                                n_confusing_labels=9,
                                                train=True)
        val_dataset, _ = get_biased_fashionmnist_dataloader(args.data_dir, batch_size=args.batch_size,
                                                              data_label_correlation=0.1,
                                                              is_foreground=True,
                                                              n_confusing_labels=9,
                                                              train=False)
        ...
        train_dataset.channels = 3
        train_dataset.im_size = (32, 32)
        # Ensure classes attribute exists if your downstream code needs it
        if not hasattr(train_dataset, 'classes'):
            train_dataset.classes = [str(c) for c in range(10)]

    elif args.dataset == 'Colored_MNIST_background':
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        normalize = transforms.Normalize(mean=mean, std=std)
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
    ...
    elif args.dataset == 'Colored_FashionMNIST_background':
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        normalize = transforms.Normalize(mean=mean, std=std)
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
...
def condense(args, logger, device='cuda'):
    ...
    for it in range(n_iter):
        ...
        if (it + 1) in it_test:
            ...
            data_save = [copy.deepcopy(synset.data.detach().cpu()), copy.deepcopy(synset.targets.cpu())]
            if args.net_type == 'convnet':
                net_type_name = 'ConvNet'
            torch.save({'data': data_save, 'accs_all_exps': [], },
                    os.path.join(args.save_dir, 'res_%s_%s_%s_%dipc.pt'%('IDC', args.dataset, net_type_name, args.ipc)))
