...
class CIFAR_10S(VisionDataset):
    def __init__(self, root, split='train', transform=None, target_transform=None,
        ...
        # Define a unique filename for this configuration
        save_filename = f"cifar10s_{split}_r{skewed_ratio}_s{seed}.pkl"
        save_path = os.path.join(root, save_filename)

        if os.path.exists(save_path):
            print(f"[CIFAR_10S] Loading cached biased dataset from: {save_path}")
            with open(save_path, 'rb') as f:
                saved_data = pickle.load(f)
                self.dataset = saved_data['dataset']
                data_count = saved_data['data_count']
        else:
            print(f"[CIFAR_10S] Generating biased dataset (skew={skewed_ratio})...")
            # Generate the data using the original function
            ...
            # Save it to disk
            print(f"[CIFAR_10S] Saving biased dataset to: {save_path}")
            with open(save_path, 'wb') as f:
                pickle.dump({'dataset': self.dataset, 'data_count': data_count}, f)
