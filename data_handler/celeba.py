...
class CelebA_train(Dataset):
    ...
    def __init__(
        ...
        landmarks_align = self._load_csv("Anno/list_landmarks_align_celeba.txt", header=1)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.read_count % 1000 == 0:
            print(f"[Log] CelebA_train loaded {self.read_count} images so far...")
        # <---------------------->
        X = Image.open(os.path.join(self.root, "Img/img_align_celeba", self.filename[index]))
...
class CelebA_test(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        self.read_count += 1
        if self.read_count % 1000 == 0:
            print(f"[Log] CelebA_test loaded {self.read_count} images so far...")
        # <---------------------->
        X = Image.open(os.path.join(self.root, "Img/img_align_celeba", self.filename[index]))
...
def CelebA(target_label_idx, sensitive_label_idx, data_dir="/home/mmoslem3/Data-distil/fairDC/FairDD/data/CelebA"):
...
if __name__ == "__main__":
    ...
    for i in range(0,40):
        ...
        train_dataset, test_dataset, mean, std = CelebA(target_label_idx, sensitive_label_idx,
                                                        data_dir="/home/mmoslem3/Data-distil/fairDC/FairDD/data/celeba")
