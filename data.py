...
# CIFAR10_S_90 (Skewed) - mapping to standard CIFAR stats
MEANS['CIFAR10_S_90'] = MEANS['cifar']
STDS['CIFAR10_S_90'] = STDS['cifar']

# Colored MNIST & FashionMNIST (Foreground & Background)
# These use mean=[0.5, 0.5, 0.5] and std=[0.5, 0.5, 0.5] to normalize to [-1, 1]
MEANS['Colored_MNIST_foreground'] = [0.5, 0.5, 0.5]
STDS['Colored_MNIST_foreground'] = [0.5, 0.5, 0.5]

MEANS['Colored_FashionMNIST_foreground'] = [0.5, 0.5, 0.5]
STDS['Colored_FashionMNIST_foreground'] = [0.5, 0.5, 0.5]

MEANS['Colored_MNIST_background'] = [0.5, 0.5, 0.5]
STDS['Colored_MNIST_background'] = [0.5, 0.5, 0.5]

MEANS['Colored_FashionMNIST_background'] = [0.5, 0.5, 0.5]
STDS['Colored_FashionMNIST_background'] = [0.5, 0.5, 0.5]
...
class MultiEpochsDataLoader(torch.utils.data.DataLoader):
    ...
    def __iter__(self):
        for i in range(len(self)):
            # Change: Capture full batch first
            batch = next(self.iterator)
            
            # Select only the first two elements (Data, Label)
            # This ignores the 3rd (sensitive) attribute to prevent crashes in current loops
            data = batch[0]
            target = batch[1]
...
def load_data(args):
    ...
    # --- INSERT NEW DATASET LOGIC HERE ---
    if args.dataset == "CIFAR10_S_90":
        # Ensure 'cifar10' module is imported or available
        train_dataset, val_dataset, mean, std = cifar10.CIFAR_10s(args.data_dir, skew_ratio=0.9)
        nclass = 10
        # args.size and args.nch are handled in argument.py

    elif args.dataset == 'Colored_MNIST_foreground':
        # Ensure get_biased_mnist_dataloader is available
